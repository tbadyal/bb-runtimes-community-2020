...
class Sam(ArmV7MTarget):
    ...
    def compiler_switches(self):
        ...
        if not self.has_single_precision_fpu:
            return base + ('-msoft-float', )
        else:
            return base + ('-mhard-float', '-mfpu=%s' % self.fpu, )

    def __init__(self, board):
        assert board in ('sam4s', 'samg55', 'samv71', 'sam3x'), \
        ...
        if self.board == 'samv71':
            ...
            self.add_gnat_sources(
                'arm/sam/sam3x/s-sam3x.ads',
                'arm/sam/%s/board_config.ads' % self.name,
                'arm/sam/%s/svd/i-sam-adc.ads' % self.name,
                'arm/sam/%s/svd/i-sam-can.ads' % self.name,
                'arm/sam/%s/svd/i-sam-chipid.ads' % self.name,
                'arm/sam/%s/svd/i-sam-dacc.ads' % self.name,
                'arm/sam/%s/svd/i-sam-dmac.ads' % self.name,
                'arm/sam/%s/svd/i-sam-ebi.ads' % self.name,
                'arm/sam/%s/svd/i-sam-efc.ads' % self.name,
                'arm/sam/%s/svd/i-sam-emac.ads' % self.name,
                'arm/sam/%s/svd/i-sam-hsmci.ads' % self.name,
                'arm/sam/%s/svd/i-sam-matrix.ads' % self.name,
                'arm/sam/%s/svd/i-sam-pio.ads' % self.name,
                'arm/sam/%s/svd/i-sam-pmc.ads' % self.name,
                'arm/sam/%s/svd/i-sam-pwm.ads' % self.name,
                'arm/sam/%s/svd/i-sam-spi.ads' % self.name,
                'arm/sam/%s/svd/i-sam-ssc.ads' % self.name,
                'arm/sam/%s/svd/i-sam-sysc.ads' % self.name,
                'arm/sam/%s/svd/i-sam-tc.ads' % self.name,
                'arm/sam/%s/svd/i-sam-trng.ads' % self.name,
                'arm/sam/%s/svd/i-sam-twi.ads' % self.name,
                'arm/sam/%s/svd/i-sam-uart.ads' % self.name,
                'arm/sam/%s/svd/i-sam-uotghs.ads' % self.name,
                'arm/sam/%s/svd/i-sam-usart.ads' % self.name)
            self.add_gnarl_sources(
                'src/s-bbpara__sam3x.ads')
        else:
            self.add_gnat_sources(
